                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
                    ...
                    <aside className="hidden lg:block lg:col-span-3 shrink-0">
                        <div className="sticky top-28 max-h-[calc(100vh-8rem)] overflow-y-auto flex flex-col space-y-6 pb-4 scrollbar-hide">
                            ...
                            {/* Navigation Tabs */}
                            <div className="bg-card/40 border border-white/5 rounded-4xl p-2 flex flex-col mt-4">
                                <button
                                    onClick={() => setActiveTab('latest')}
                                    className={`flex items-center gap-3 px-5 py-3.5 rounded-3xl text-[14px] font-medium transition-all ${activeTab === 'latest' ? 'bg-primary text-primary-foreground font-semibold shadow-[0_4px_14px_0_rgba(249,115,22,0.39)]' : 'text-stone-400 hover:text-stone-200'}`}
                                >
                                    Latest
                                </button>
                                <button
                                    onClick={() => setActiveTab('popular')}
                                    className={`flex items-center gap-3 px-5 py-3.5 rounded-3xl text-[14px] font-medium transition-all ${activeTab === 'popular' ? 'bg-primary text-primary-foreground font-semibold shadow-[0_4px_14px_0_rgba(249,115,22,0.39)]' : 'text-stone-400 hover:text-stone-200'}`}
                                >
                                    Popular
                                </button>
                                <button
                                    onClick={() => setActiveTab('older')}
                                    className={`flex items-center gap-3 px-5 py-3.5 rounded-3xl text-[14px] font-medium transition-all ${activeTab === 'older' ? 'bg-primary text-primary-foreground font-semibold shadow-[0_4px_14px_0_rgba(249,115,22,0.39)]' : 'text-stone-400 hover:text-stone-200'}`}
                                >
                                    Older
                                </button>
                            </div>
                    ...
                    <main className="lg:col-span-6 min-w-0">
                        ...
                        {/* Stories Feed */}
                        {loading ? (
                            <div className="flex flex-col items-center justify-center py-32 rounded-4xl border border-white/5 bg-card/40">
                                <Loader2 className="h-10 w-10 animate-spin text-primary mb-4" />
                                <p className="text-stone-400 font-medium">Loading stories...</p>
                            </div>
                        ) : stories.length === 0 ? (
                            <div className="flex flex-col items-center justify-center py-32 px-4 rounded-4xl border border-white/5 border-dashed bg-card/20">
                                <div className="h-16 w-16 rounded-3xl bg-white/5 border border-white/10 flex items-center justify-center mb-6 shadow-sm">
                                    <BookOpen className="h-8 w-8 text-stone-500" strokeWidth={1.5} />
                                </div>
                                <h3 className="text-xl font-bold text-white mb-2 tracking-tight">No Stories Yet</h3>
                                <p className="text-stone-400 text-center max-w-md font-medium text-[15px]">
                                    {isAuthenticated
