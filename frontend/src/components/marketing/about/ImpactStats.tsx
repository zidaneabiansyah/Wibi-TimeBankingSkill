  ...
  return (
    ...
    >
      <div className="max-w-5xl mx-auto flex flex-col gap-16">
        ...
        <div
          className={`space-y-6 transition-all duration-1000 ease-out ${visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"
            }`}
        >
          ...
          <h2 className="text-3xl md:text-4xl lg:text-[40px] font-light leading-[1.3] text-foreground">
            Wibi adalah platform edukasi yang membantu para pelajar{" "}
            <em className="font-semibold italic text-foreground" style={{ fontFamily: "serif" }}>
              menemukan mentor
            </em>
            ,{" "}
            <em className="font-semibold italic text-foreground" style={{ fontFamily: "serif" }}>
        ...
        {/* Bottom: Stats Row */}
        <div
          className={`grid grid-cols-2 md:grid-cols-4 gap-y-12 transition-all duration-1000 delay-300 ease-out ${visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-12"
            }`}
        >
          {stats.map((stat, idx) => (
            <div
              key={idx}
              className={`flex flex-col ${idx !== 0 ? "md:border-l md:border-border md:pl-8" : ""
                } ${idx % 2 !== 0 ? "border-l border-border pl-4 md:pl-8" : "pr-4 md:pr-0" // Mobile 2-col handling
                }`}
            >
