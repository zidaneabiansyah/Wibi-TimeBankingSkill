...
export function CustomLeaderboard() {
    ...
    return (
        <div className="w-full flex flex-col items-center">
            ...
            <div className="w-full max-w-5xl mx-auto flex flex-col md:flex-row items-center justify-between bg-stone-950 p-2 md:p-3 rounded-2xl md:rounded-full border border-stone-800 mb-16 shadow-inner">
                {/* Categories */}
                <div className="flex flex-wrap items-center gap-1 md:gap-2 w-full md:w-auto overflow-x-auto no-scrollbar pb-2 md:pb-0">
                    {CATEGORIES.map((cat) => (
                        <button
                            ...
                            className={`flex items-center gap-2 px-4 md:px-6 py-2 md:py-2.5 rounded-xl md:rounded-full text-sm font-semibold transition-all duration-300 min-w-max ${
                ...
                {/* Dropdown Range */}
                <div className="flex items-center gap-3 px-4 md:px-6 py-2 mt-2 md:mt-0 w-full md:w-auto justify-between border-t border-stone-800 md:border-none">
                    <span className="text-sm font-medium text-stone-400 whitespace-nowrap">Time Range:</span>
                    <div className="relative min-w-[120px]">
                        <select 
                            value={activeTimeRange}
                            onChange={(e) => handleTimeRangeChange(e.target.value as TimeRange)}
                            className="w-full bg-stone-900 border border-stone-800 text-white text-sm font-bold rounded-lg md:rounded-xl px-4 py-2.5 pr-10 appearance-none focus:outline-none focus:ring-2 focus:ring-primary/50 cursor-pointer shadow-sm transition-colors hover:border-stone-700"
                        >
            ...
            {/* Podium Layout */}
            <div 
                className={`flex flex-col md:flex-row items-end justify-center w-full max-w-4xl mx-auto transition-all duration-300 transform ${
            ...
            >
                {isLoading && sortedData.length === 0 ? (
                    <div className="flex flex-col items-center justify-center py-20 w-full h-80">
                        ...
                        <span className="text-stone-400 font-medium">Memuat data papan skor...</span>
                    </div>
                ) : (
                    displayOrder.map((user, idx) => {
                        ...
                        <div 
                            ...
                            className={`flex flex-col items-center px-2 w-full md:auto ${
                                isRank1 ? 'order-1 md:order-2 z-10 md:w-[35%]' : idx === 0 ? 'order-2 md:order-1 md:w-[30%] -mb-2 md:mb-0' : 'order-3 md:order-3 md:w-[30%] -mb-2 md:mb-0'
                            }`}
                        >
                            <div 
                                className={`w-full relative flex flex-col items-center bg-stone-900 backdrop-blur-sm transition-all duration-300 ${
                                    isRank1 
                                        ? 'rounded-[2.5rem] border border-primary/40 p-10 pb-12 shadow-2xl shadow-primary/20 scale-105 transform origin-bottom' 
                                        : 'rounded-3xl border border-stone-800 p-8 opacity-90 hover:opacity-100 shadow-xl'
                                }`}
                            >
                                ...
                                <div className="relative mb-6">
                                    <div 
                                        className={`relative overflow-hidden ring-4 ring-offset-4 ring-offset-stone-900 shadow-lg ${
                                            isRank1 ? 'w-28 h-28 ring-yellow-500 shadow-yellow-500/30' : 'w-20 h-20 ring-stone-700'
                                        }`}
                                    ...
                                    {/* Rank Badge */}
                                    <div className={`absolute -bottom-4 left-1/2 -translate-x-1/2 w-8 h-8 rounded-full flex items-center justify-center font-black text-sm shadow-xl border-4 border-stone-900 ${
                                        isRank1 ? 'bg-yellow-500 text-stone-900' : 
                                ...
                                <h3 className={`font-black text-center text-white truncate w-full px-2 ${isRank1 ? 'text-2xl mt-2' : 'text-xl mt-1'}`}>
            ...
            <div className={`w-full max-w-5xl mx-auto mt-20 transition-all duration-300 ${fade ? 'opacity-0 translate-y-4' : 'opacity-100 translate-y-0'}`}>
                {/* Table Header */}
                <div className="hidden md:flex items-center px-12 py-3 text-xs font-bold text-stone-500 uppercase tracking-wider mb-2">
                ...
                <div className="flex flex-col gap-3 px-4 md:px-0 pb-16">
                    {others.map((user) => {
                        ...
                        <div key={user.user_id + "_" + user.rank} className="relative group cursor-pointer w-[95%] md:w-full mx-auto max-w-[95vw]">
                            ...
                            <div className="absolute inset-x-0 inset-y-0 bg-stone-900 border border-stone-800 rounded-lg md:rounded-xl md:-skew-x-[15deg] group-hover:border-primary/50 group-hover:bg-primary/5 transition-all duration-200" />
                            ...
                            <div className="relative flex flex-col md:flex-row items-center py-4 px-6 md:px-10 min-h-[5rem]">
                                ...
                                <div className="flex items-center gap-4 w-full md:w-64">
                                    <span className="text-xl font-black text-stone-500 w-8 md:w-10 tabular-nums">{user.rank}</span>
                                    <div className="relative w-12 h-12 rounded-full overflow-hidden shrink-0 border border-stone-700">
                                        <Image src={avatarUrl} alt={user.username} fill className="object-cover" unoptimized/>
                                    </div>
                                    <div className="flexflex-col">
                                        <div className="flex items-center gap-1.5">
                                            <span className="font-bold text-white text-base md:text-sm whitespace-nowrap">{user.full_name || user.username}</span>
                                        ...
                                        <span className="text-xs font-medium text-stone-500 hidden md:block">{roleTitle}</span>
                                    </div>
                                </div>

                                {/* Divider & Tags */}
                                <div className="hidden md:flex items-center pl-6 pr-4 ml-4 lg:ml-0 gap-3">
                                    <span className="text-xs bg-stone-800/80 text-stone-300 px-3 py-1.5 rounded-full font-bold flex items-center gap-1.5 whitespace-nowrap">
                                        <Gem className="w-3 h-3 text-primary" /> Badge Holder
                                    </span>
                                </div>

                                {/* Stats Area (Right side) */}
                                <div className="flex items-center justify-between md:justify-end gap-6 md:gap-0 mt-4 md:mt-0 w-full md:flex-1 text-sm font-bold text-stone-300">
                                    <div className="text-center md:w-24"><span className="md:hidden text-stone-500 font-normal mr-2">Level:</span>Lv.{userLevel}</div>
                                    <div className="text-center md:w-24"><span className="md:hidden text-stone-500 font-normal mr-2">Helps:</span>{helpsGiven}</div>
                                    <div className="text-center md:w-24"><span className="md:hidden text-stone-500 font-normal mr-2">Impact:</span>{impactRate}</div>
            ...
            {/* Bottom Info Bar */}
            <div className="w-full mt-12 pt-6 border-t border-stone-800/60 flex justify-center text-center">
                {user ? (
                    <p className="text-sm text-stone-400 font-medium">
                        Kamu mendapatkan <span className="text-primary font-bold">{userPointsStr} poin</span> dengan kategori ini, dan berada di peringkat <span className="text-white font-bold max-w-fit px-2 py-0.5 rounded bg-stone-800">{userRankStr}</span> dari {(totalUsersDummy).toLocaleString()} pengguna.
                    </p>
                ) : (
                    <p className="text-sm text-stone-400 font-medium">
