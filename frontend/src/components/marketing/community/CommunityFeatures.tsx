...
export function CommunityFeatures() {
    ...
    return (
        <section ref={sectionRef} className="bg-stone-950 py-24 px-6 md:px-12 w-full">
            <div className="max-w-7xl mx-auto">
                {/* Part 1: Heading */}
                <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 mb-10">
                    <h2 className="text-5xl md:text-6xl font-medium text-white leading-tight tracking-tight">
                        {headingText}
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 divide-y md:divide-y-0 md:divide-x divide-stone-800">
                    ...
                    {features.map((feature, idx) => (
                        <Link href={feature.href} key={idx} className="relative overflow-hidden block pt-8 px-4 md:px-6 pb-10 flex-col group cursor-pointer hover:bg-stone-900/60 transition-colors duration-500">
                            ...
                            <h3 className="relative z-10 text-xl font-semibold text-white mb-4">{feature.title}</h3>
                            <div className="relative z-10 space-y-1 mb-6">
                                {feature.actions.map((action, aIdx) => (
                                    <div key={aIdx} className="flex items-center gap-2 text-sm text-stone-400 hover:text-primary cursor-pointer py-1 transition-colors">
                            ...
                            <div className="relative z-10 border-t border-stone-800 my-6 group-hover:border-primary/20 transition-colors duration-500" />
                            
                            <div className="relative z-10 mt-auto">
                                <h4 className="text-xs font-semibold text-stone-500 uppercase tracking-widest mb-2 group-hover:text-primary/70 transition-colors duration-500">Tentang Fitur Ini</h4>
                                <p className="text-xs text-stone-400 leading-relaxed group-hover:text-stone-300 transition-colors duration-500">
                                    {feature.about}
                                </p>
                            </div>
                        </Link>
